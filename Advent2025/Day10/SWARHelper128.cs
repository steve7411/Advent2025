...
internal class SWARHelper128 : ISWARHelper<UInt128> {
    ...
    public static readonly UInt128 LEAST_SIGNIFICANT_DIGIT_MASK = ((UInt128)1 << ISWARHelper<UInt128>.SPACING) - UInt128.One;

    static UInt128 ISWARHelper<UInt128>.BORROW_MASK => BORROW_MASK;

    static unsafe SWARHelper128() {
        const int TOTAL_WIDTH = 128;
        var width = TOTAL_WIDTH - TOTAL_WIDTH % ISWARHelper<UInt128>.SPACING;
        PARITY_MASK = UInt128.One;
        for (var i = ISWARHelper<UInt128>.SPACING; i < width; i += ISWARHelper<UInt128>.SPACING)
            PARITY_MASK |= UInt128.One << i;

        BORROW_MASK = PARITY_MASK << ISWARHelper<UInt128>.SPACING - 1;
    ...
    public static UInt128 Expand(ReadOnlySpan<int> ints) {
        ...
        for (var i = ints.Length - 1; i >= 0; --i) {
            Debug.Assert(ints[i] <= (1 << ISWARHelper<UInt128>.SPACING - 1) - 1);
            res = res << ISWARHelper<UInt128>.SPACING | (UInt128)(uint)ints[i];
        }
        return res;
    }
